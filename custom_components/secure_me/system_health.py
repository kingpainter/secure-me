...
async def async_system_health_info(hass: HomeAssistant) -> dict[str, Any]:
    ...
    # Module health
    module_manager = coordinator.module_manager
    enabled_modules = [m for m in module_manager.modules.values() if m.enabled]
    health_info["modules_enabled"] = len(enabled_modules)
    health_info["modules_total"] = len(module_manager.modules)
    
    # Check module health
    unhealthy_modules = []
    for module_id, module in module_manager.modules.items():
