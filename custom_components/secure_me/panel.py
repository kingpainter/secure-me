...
# ═══════════════════════════════════════════════════════════════════════════════════
# TEMPORARY TEST CODE -- remove this whole block (plus test-module-a.js and
# test-module-b.js in frontend/) once the panel.js ES-module split
# feasibility test is done (see chat, 2026-08-23).
#
# Registers a throwaway "SM ES Test" sidebar panel (admin-only) that loads
# test-module-b.js, which in turn `import`s from test-module-a.js. If the
# resulting panel shows the green success message, panel_custom's module_url
# loading supports native ES module import/export between two separately-
# served static files without a bundler -- meaning panel.js CAN be safely
# split into multiple files. If the panel is blank, shows a raw import error
# in the browser console, or the sidebar entry never appears, it can't (or
# not without a bundler/build step).
# ═══════════════════════════════════════════════════════════════════════════════════

_TEST_A_URL = f"/api/{DOMAIN}-es-test-a"
_TEST_B_URL = f"/api/{DOMAIN}-es-test-b"
_TEST_A_FILENAME = "test-module-a.js"
_TEST_B_FILENAME = "test-module-b.js"


async def async_register_es_module_test_panel(hass: HomeAssistant) -> None:
    """TEMPORARY: register the ES-module split feasibility test panel.

    Safe no-op if the test files aren't present (e.g. already cleaned up).
    """
    hass.data.setdefault(DOMAIN, {})
    if hass.data[DOMAIN].get("_es_test_registered", False):
        return

    root_dir = hass.config.path(CUSTOM_COMPONENTS, DOMAIN)
    panel_dir = os.path.join(root_dir, PANEL_FOLDER)
    test_a_file = os.path.join(panel_dir, _TEST_A_FILENAME)
    test_b_file = os.path.join(panel_dir, _TEST_B_FILENAME)

    if not (os.path.isfile(test_a_file) and os.path.isfile(test_b_file)):
        _LOGGER.debug("Secure Me: ES module test files not found, skipping test panel")
        return

    try:
        await hass.http.async_register_static_paths([
            StaticPathConfig(_TEST_A_URL, test_a_file, cache_headers=False),
            StaticPathConfig(_TEST_B_URL, test_b_file, cache_headers=False),
        ])
        await panel_custom.async_register_panel(
            hass,
            webcomponent_name="secure-me-module-test",
            frontend_url_path="secure_me_es_test",
            module_url=f"{_TEST_B_URL}?v=1",
            sidebar_title="SM ES Test",
            sidebar_icon="mdi:flask",
            require_admin=True,
            config={},
        )
        hass.data[DOMAIN]["_es_test_registered"] = True
        _LOGGER.info(
            "Secure Me: ES module test panel registered -- check sidebar for 'SM ES Test'"
        )
    except Exception as err:
        _LOGGER.warning("Secure Me: ES module test panel registration failed: %s", err)
# ═══════════════════════════════════════════════════════════════════════════════════
