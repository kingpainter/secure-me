...
async def ws_get_users(
    ...
    for uid, user in users.items():
        masked[uid] = {**user, "code": "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢" if user.get("code") else ""}
...
async def ws_test_notification(
    ...
    try:
        ...
        service_data = {
            ...
            "title": f"ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂºÃ‚Â¡ÃƒÂ¯Ã‚Â¸Ã‚Â Secure Me Test: {notif.get('name', 'Test')}",
        }
...
def _get_module_entity_ids(module) -> list[str]:
    ...
    if not entities and hasattr(module, 'config'):
        ...
        for key in ("cameras", "locks", "climates", "lights", "media_players"):
